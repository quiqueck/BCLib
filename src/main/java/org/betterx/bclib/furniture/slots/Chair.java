...
/**
 * Builds a chair slot: a {@link BaseChair} derived from the set's slab, with a model generated from the set's
 * planks texture and a {@code "I "/"##"/"II"} recipe from slab + sticks.
 * <p>
 * The generated model needs no dedicated textures - {@link BCLModels#createChairBlockModel} re-textures the
 * shared {@code bclib:block/chair} model with the source block's texture.
 */
public class Chair extends SlotFromDefinition {
    /** Resolves the cloth block whose texture the chair's upholstery uses. */
    protected final Supplier<Block> cloth;
    /** The slot the chair's block properties, behaviour and recipe material are taken from. */
    protected final SlotType sourceSlot;
    /** The slot whose texture the generated model uses. */
    protected final SlotType textureSlot;

    /**
     * Creates a factory for the {@link SlotType#CHAIR} slot, built from the set's {@link SlotType#SLAB} and
     ...
     */
    public Chair(Supplier<Block> cloth) {
        this(SlotType.CHAIR, SlotType.SLAB, SlotType.PLANKS, cloth);
    }

    /**
     * @param slot        the slot to register this chair under
     ...
     * @param cloth       resolves the cloth block, deferred so it may be configured after this factory is created
     */
    public Chair(SlotType slot, SlotType sourceSlot, SlotType textureSlot, Supplier<Block> cloth) {
        super(slot);
    ...
    private static class ClientModel {
        private static BlockModelTrait build(BlockSet<?> set, SlotType textureSlot, Supplier<Block> cloth) {
            return ClientBlockTraits.MODEL.with((key, block, generator) -> BCLModels.createChairBlockModel(
                    generator,
                    block,
