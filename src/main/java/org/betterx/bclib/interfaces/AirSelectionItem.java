...
public interface AirSelectionItem {
    ...
    default InteractionResult useOnAir(Level level, Player player, InteractionHand interactionHand) {
        final BlockHitResult hit = getAirSelectionHit(level, player);

        if (hit != null) {
            return this.useOn(new UseOnContext(player, interactionHand, hit));
        }
